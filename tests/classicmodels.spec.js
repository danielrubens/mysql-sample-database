...
describe('MySQL Sample Database Tests', () => {
    ...
    it('Return all Employees who report to Anthony Bow, Sales Manager', async () => {
        const query = readFileSync('01.employees_report.sql', 'utf-8');
    ...
    it('Find all Customers and theirs orders that were shipped in August 2003', async () => {
        const query = readFileSync('02.customers_orders_shipped.sql', 'utf-8');
    ...
    it('List only the products with orders where products have been ordered by the price of 100 to 150', async () => {
        const query = readFileSync('03.products_orders_price.sql', 'utf-8');
